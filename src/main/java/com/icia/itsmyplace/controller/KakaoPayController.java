...
//				}
    ...
    {		
		...
		try {
			...
			//카페운영자에게 메일전송(예약들어왔다고)
			if(!StringUtil.isEmpty(cafeUserMail) && !StringUtil.isEmpty(userMail)) { // 카페운영자에게 예약정보 메일로 전송하기
				String subject = "[내자리얌] "+ cafe.getCafeName() + " 사장님, 환불된 예약건을 확인하세요.";
				String content = "예약번호 : " + rsrvSeq + "\n"
								+ "예약자 아이디 : " + rsRv.getUserId() + "\n"
								+ "예약일 : " + rsRv.getRsrvDate() + "\n"
								+ "예약시간 : " + userRsrvTime + "\n"
				...
				String subject_U = "[내자리얌] " + user.getUserName() +"님, 환불된 예약건을 확인하세요.";
				String content_U = "예약번호 : " + rsrvSeq + "\n"
								+ "예약자 아이디 : " + rsRv.getUserId() + "\n"
								+ "예약일 : " + rsRv.getRsrvDate() + "\n"
								+ "예약시간 : " + userRsrvTime + "\n"
								+ "예약인원수 : " + rsRv.getRsrvPplCnt() + "\n"
				...
				MimeMessage mail_U = mailSender.createMimeMessage();
				MimeMessageHelper mailHelper_U = new MimeMessageHelper(mail_U, true, "UTF-8");
				
				mailHelper_U.setSubject(subject_U);
				mailHelper_U.setText(content_U);
				mailHelper_U.setFrom(from);
				mailHelper_U.setTo(userMail);
				...
				mailSender.send(mail);
				mailSender.send(mail_U);
				ajaxResponse.setResponse(0, "success");
			}
			else {
				ajaxResponse.setResponse(600, "Email is Null (cafe)");
			}
...
//	@RequestMapping(value="/kakao/payRetry")
//	@ResponseBody
//	public Response<Object> payRetry(HttpServletRequest request, HttpServletResponse response) throws Exception
//	{
//		long rsrvSeq = Integer.parseInt(HttpUtil.get(request, "rsrvSeq"));
//		
//		Response<Object> ajaxResponse = new Response<Object>();
