               ...
               <div class="col-md-4">
                  <div class="index-eventNameTag">
                     <c:set var="titleLength" value="${fn:length (eventList.bbsTitle)}" />
                     	<c:choose>
							<c:when test="${eventList.userId eq 'cafeNoname'}">
								<div class="index-eventNameText" id="${eventList.bbsSeq}bbsTitle">
									<c:set var="title" value="${eventList.bbsTitle}" />
									${fn:substring(title,0,5)}<br/>
									${fn:substring(title,6,16)}
									<c:if test="${titleLength > 16}">...</c:if>
								</div>
							</c:when>
							<c:otherwise>
								<div class="index-eventNameText" id="${eventList.bbsSeq}bbsTitle">
									<c:set var="title" value="${eventList.bbsTitle}" />
									${fn:substring(title,0,6)}<br/>
									${fn:substring(title,7,17)}
									<c:if test="${titleLength > 17}">...</c:if>
								</div>
							</c:otherwise>
						</c:choose>              
